...
function monte_carlo(m,n,k, n_samples)
    # TODO: use play_game(m,n,k) defined below to compute the Monte Carlo estimate.
end
...
function recurse(k,board,moves,player)
    ...
    for idx = 1:n_moves
        ...
        # TODO: recursively determine the outcome probabilities given the
        # current state of `board`, and update `prob` accordingly.
        # Hint: you will need to call `winning_move()` and `recurse()`.
